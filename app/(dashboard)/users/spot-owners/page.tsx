...
export default function SpotOwnersPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Spot Owner List"
        breadcrumb={["Dashboard", "User List", "Spot Owner List"]}
        action={<SearchBox value={search} onChange={handleSearchChange} placeholder="Search for Spot Owner" />}
      />
            ...
            <Table>
              <TableHeader className="bg-white">
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="pr-8 text-right">Active Event</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {users.length === 0 ? (
                  ...
                  users.map((user) => (
                    <TableRow key={user._id}>
                      ...
                      <TableCell>{formatDateLong(user.createdAt)}</TableCell>
                      <TableCell>{user.email}</TableCell>
                      <TableCell>{user.phone || "-"}</TableCell>
                      <TableCell className="pr-8 text-right font-medium">{user.activeEvents || 0}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
