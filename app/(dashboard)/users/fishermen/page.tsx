...
export default function FishermenPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Fisherman List"
        breadcrumb={["Dashboard", "User List", "Fisherman List"]}
        action={<SearchBox value={search} onChange={handleSearchChange} placeholder="Search for Fisherman" />}
      />
            ...
            <Table>
              <TableHeader className="bg-white">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="pl-8">Fisherman Name</TableHead>
                  <TableHead>Phone Number</TableHead>
                  <TableHead>Email</TableHead>
                  <TableHead>Total Bookings</TableHead>
                  <TableHead className="pr-8 text-right">Total Paid Amount</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {users.length === 0 ? (
                  ...
                  users.map((user) => (
                    <TableRow key={user._id}>
                      ...
                      <TableCell className="pr-8 text-right font-medium">
                        {formatMoney(user.totalPaidAmount || 0)}
                      </TableCell>
                    </TableRow>
                  ))
                )}
